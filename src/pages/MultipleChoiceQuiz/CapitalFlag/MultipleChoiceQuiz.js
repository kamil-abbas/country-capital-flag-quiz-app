...
const MultipleChoiceQuiz = () => {
  ...
  let questionDetails = useSelector(store => store.capitalFlagQuizReducer.questionDetails);
  let results = useSelector(store => store.capitalFlagQuizReducer.results);
  ...
  setTimeout(() => {
    document.querySelectorAll(".cap-fla-answer").forEach(item => {if (item.classList.contains("hidden")){item.classList.remove("hidden")}})  
  }, 2000);

  if(questionNum > 10){
    console.log(document.querySelector('.cap-fla-question-title'));
    document.querySelector('.cap-fla-question-title').style.flexDirection = "column";
    document.querySelector('.cap-fla-question-title').innerHTML = "Thank you for playing the quiz. Results are ready. <a href='/multiple-choice-quiz-capital-flag'>Start new game</a> <a href='/multiple-choice-quiz-capital-flag/results'>See Results</a>";
  }
  ...
  function handleStartClick(e) {
    ...
    e.target.classList.add('cap-fla-animation-for-start');
  ...
  function handleHelpClick() {
    ...
    if (helpNumInQuestion == 0 && answerClickNum != 1 && helpNumInTotal < 3) {
      document.querySelectorAll(".cap-fla-answer").forEach((item, index) => {
  ...
  function handleShowClick() {
    if (answerClickNum < 1) {
      playWrong();
      document.querySelectorAll(".cap-fla-answer").forEach((answer) => {
        ...
        if (answer.firstChild.src == correctAnswer) {
          ...
          setTimeout(() => {
            ...
            dispatch(ifWrong(answerArray[randNum].capital, "", correctAnswer, rnfs, answerArray[randNum].country));
    ...
    if (answerClickNum == 1) {
      ...
      setTimeout(() => {
        if (e.target.firstChild.src == correctAnswer) {
          ...
          setTimeout(() => {
            dispatch(ifCorrect(answerArray[randNum].capital, e.target.firstChild.src, correctAnswer, rnfs, answerArray[randNum].country));
          ...
          setTimeout(() => {
            dispatch(ifWrong(answerArray[randNum].capital, e.target.firstChild.src, correctAnswer, rnfs, answerArray[randNum].country));
            setquestionNum(preNum => preNum + 1);
            setTimeStarted(true);
  ...
  {
    return (
      <div className="cap-fla-multiple-choice-container">
        <div className="cap-fla-horizontal-center">
          <div className="cap-fla-start-btn" onClick={(e) => {handleStartClick(e)}}>
            Start
          </div>
        </div>
        <div className="cap-fla-game-wrapper hidden">
          <div className="cap-fla-question-wrapper">
            <div className="cap-fla-question-container">
              <div className="cap-fla-question-title">
                {questionNum}. Find the flag according to the capital city: {answerArray[randNum].capital}
              </div>
              <div className="cap-fla-choice-wrapper">
                <div className="cap-fla-choice-container">
                  {answerArray.map((item, index) => {
                    return (
                      <div key={index} className={questionNum > 10 ? "hidden" : "cap-fla-answer cap-fla-answer-hover"} onClick={(e) => handleAnswerClick(e)}>
          ...
          <aside className="cap-fla-aside">
            <div className="cap-fla-aside-left">
              <div className="cap-fla-aside-item-wrapper">
                <div
                  className={helpNumInTotal < 3
                      ? "cap-fla-aside-item-container cap-fla-aside-left-margin-top"
                      : "cap-fla-aside-item-container cap-fla-aside-left-margin-top not-clickable"
                  }
              ...
              <div className="cap-fla-aside-item-wrapper">
                <div className="cap-fla-aside-item-container cap-fla-aside-left-margin-top" onClick={(e) => handleShowClick()}>
                  <ShowAnswer />
                </div>
              </div>
            </div>
            <div className="cap-fla-aside-right">
              <div className="cap-fla-aside-item-wrapper">
                <div className="cap-fla-aside-item-container">
              ...
              <div className="cap-fla-aside-item-wrapper">
                <div className={questionNum > 10 ? "cap-fla-aside-item-container" : "hidden"} onClick={handleResult()}>
                  <SeeResults />
                </div>
              </div>
            </div>
